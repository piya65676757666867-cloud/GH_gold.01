...
def strip_html(text: str) -> str:
    """
    Cleans HTML markup from RSS feeds summary / description:
    1. Replaces </p>, </li>, <br>, <br/> with space to avoid sentence merging.
    2. Removes remaining HTML tags (<[^>]+>).
    3. Removes trailing incomplete HTML tags (<[^<>]*$).
    4. Unescapes HTML entities (&amp;, &#39;, &quot;, etc.).
    5. Collapses multiple whitespace into a single space and trims.
    """
    if not text:
        return ""

    # 1. Replace closing tags and line breaks with space
    text = re.sub(r'</p>|</li>|<br\s*/?>', ' ', text, flags=re.IGNORECASE)
    # 2. Remove remaining complete tags
    text = re.sub(r'<[^>]+>', ' ', text)
    # 3. Remove trailing incomplete tags (e.g. `<a...` cut off at end without `>`)
    text = re.sub(r'<[^<>]*$', '', text)
    # 4. Decode HTML entities
    text = html.unescape(text)
    # 5. Collapse duplicate whitespace & trim
...
def truncate_summary(summary: str, max_length: int = 280) -> str:
    """
    Truncates text to at most max_length characters without breaking words.
    """
    if not summary:
...
# ==========================================
# 4. Relevance Scoring & Timezone Functions
# ==========================================

def calculate_relevance(title: str, summary: str) -> Tuple[int, List[str]]:
    """
    Calculates total relevance score and matched keywords using word-boundary regex matching.
    """
...
def parse_entry_datetime(entry: dict) -> Optional[datetime.datetime]:
    """
    Parses datetime from RSS entry using email.utils or feedparser struct_time fallback.
    Returns datetime object with UTC tzinfo, or None if unparseable.
    """
...
def format_datetime_pair(dt: Optional[datetime.datetime], source_tz_str: str) -> Tuple[str, str]:
    """
    Formats a datetime object into Bangkok time and source local time strings.
    """
    if dt is None:
...
def deduplicate_news(news_items: List[dict], similarity_threshold: float = SIMILARITY_THRESHOLD) -> List[dict]:
    """
    Groups similar news articles across sources using difflib.SequenceMatcher.
    Retains:
    - Max relevance score in group
    - List of confirming sources
    - Confidence score (number of sources)
    - Earliest published datetime (first reported time)
    - Union of unique matched keywords
    - Longest title as the representative title
    """
    ...
    deduped = []
    for group in groups:
        # Representative item has the longest title
    ...
    # Sort groups by (score, confidence) descending
    deduped.sort(key=lambda x: (x["score"], x["confidence"]), reverse=True)
    return deduped


# ==========================================
# 6. Fetch & Console Display
# ==========================================

def fetch_and_filter() -> Tuple[List[dict], List[dict]]:
    """
    Fetches RSS feeds from all defined sources, filters by MIN_SCORE, prints per-source entries,
    and returns (raw_filtered_news, deduped_news).
    """
    raw_filtered_news = []
    ...
    # Deduplicate after fetching all sources
...
def export_to_json(deduped_news: List[dict], output_path: str = "gold_news_for_ai.json", top_n: int = 15) -> str:
    """
    Exports top_n news items to a JSON file formatted for downstream AI context.
...
# ==========================================
# 8. Self-Test Suite (Self-Check Requirements)
# ==========================================

def run_self_tests():
    """
    Runs automated self-checks to verify HTML stripping, regex word boundary matching,
    and news deduplication behavior.
    """
    ...
    print("  ✅ 1. strip_html() ผ่านการทดสอบ (ลบ HTML tags, <br>, และ tag ค้างท้ายข้อความได้ถูกต้อง)")

    # Test 2: calculate_relevance regex word boundary matching ("war" vs "toward")
    score_toward, kw_toward = calculate_relevance("Moving toward a new economic policy", "")
    assert "war" not in kw_toward, "calculate_relevance failed! 'war' falsely matched inside 'toward'"
    print("  ✅ 2. calculate_relevance() ผ่านการทดสอบ (regex \\bwar\\b ไม่แมตช์คำว่า 'toward')")

    score_war, kw_war = calculate_relevance("The war in Ukraine impacts Fed interest rate decisions", "")
    assert "war" in kw_war and "ukraine" in kw_war and "fed" in kw_war and "interest rate" in kw_war, "calculate_relevance failed to match exact keywords"
    ...
    print("  ✅ 3. calculate_relevance() ผ่านการทดสอบ (คำนวณคะแนนและคำสำคัญได้ถูกต้อง)")

    # Test 3: deduplicate_news
    sample_news = [
        {
            "source": "CNBC Economy",
            "source_tz": "America/New_York",
            "title": "US Debt Hits $40 Trillion Mark",
            ...
            "bkk_time_str": "19/08/2026 17:00 น.",
            "local_time_str": "19/08/2026 06:00 (America/New_York)",
        },
        {
            "source": "BBC Business",
            "source_tz": "Europe/London",
            "title": "US National Debt Hits $40 Trillion Level",
            ...
            "bkk_time_str": "19/08/2026 16:30 น.",
            "local_time_str": "19/08/2026 10:30 (Europe/London)",
        },
        {
            "source": "Financial Times",
            "source_tz": "Europe/London",
            "title": "US Debt Reaches Historic $40 Trillion Level Today",
            "summary": "Detailed summary from FT",
            "link": "https://ft.com/1",
            "score": 9,
            "matched_keywords": ["debt", "us debt"],
            "published_dt": datetime.datetime(2026, 8, 19, 11, 0, tzinfo=datetime.timezone.utc),
            "bkk_time_str": "19/08/2026 18:00 น.",
            "local_time_str": "19/08/2026 12:00 (Europe/London)",
        },
    ]

    deduped = deduplicate_news(sample_news, similarity_threshold=0.5)
    assert len(deduped) == 1, f"deduplicate_news failed! Expected 1 group, got {len(deduped)}"
    assert deduped[0]["confidence"] == 3, f"Expected confidence 3, got {deduped[0]['confidence']}"
    assert deduped[0]["score"] == 10, f"Expected max score 10, got {deduped[0]['score']}"
    assert deduped[0]["title"] == "US Debt Reaches Historic $40 Trillion Level Today", "Representative title should be the longest title"
    assert "CNBC Economy" in deduped[0]["sources"] and "BBC Business" in deduped[0]["sources"] and "Financial Times" in deduped[0]["sources"]
    assert deduped[0]["published_dt"] == datetime.datetime(2026, 8, 19, 9, 30, tzinfo=datetime.timezone.utc), "Should preserve earliest publication time"
    print("  ✅ 4. deduplicate_news() ผ่านการทดสอบ (รวมข่าวซ้ำ 3 สำนักเป็น 1 กลุ่ม พร้อม confidence = 3 และเวลาเก่าที่สุด)")
...
def main():
    ...
    # Always run self-tests first to ensure reliability
    run_self_tests()
